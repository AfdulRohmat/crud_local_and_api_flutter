...
class CrudApiRepository {
  ...
  Future<dynamic> fetchDetailProductsApi(
      Map<String, String>? headers, int id) async {
    try {
      dynamic response = await _apiServices.getApiResoponse(
          "${AppUrl.baseFakeStoreApi}products/$id", headers);

      return response = detailProductlModelFromJson(response);
    } catch (e) {
      debugPrint("error data repo : $e");
      rethrow;
    }
